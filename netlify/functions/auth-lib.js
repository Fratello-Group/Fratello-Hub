...
async function legacyCodeRole(code) {
    ...
    const temporaryOwnerRescueExpiresAt = Date.UTC(2026, 4, 9, 6, 0, 0);
    if (
        Date.now() < temporaryOwnerRescueExpiresAt &&
        (normalized === 'fratello-owner-rescue' || normalized === 'fratello-owner-setup')
    ) {
